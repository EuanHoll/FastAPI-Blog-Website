...
import requests
from blog_website.admin_views import (
    BlogPostView,
    ContactSubmissionView,
    UserTokenView,
    UserView,
)
from blog_website.auth import AdminAuth
from blog_website.models import User
from blog_website.services import (
    BlogPostService,
    ContactFormService,
    EmailService,
...
with defaults.Session() as session:
    if not session.query(User).filter(User.admin == True).first():
        admin_user = User(username="admin", admin=True, email="admin@admin.com")
...
async def contact_form_submission(
    ...
    try:
        ContactFormService.submit(name, email, html.escape(message))
        ...
        background_tasks.add_task(
            ...
            # Include an escaped version of the message for the HTML email
